...
sub run {
    ...
    script_run("sleep 3m", timeout => 200, die_on_timeout => 0);
    script_run("date");
    systemctl('start sshd');
    script_run("date");
    script_run("sleep 197", timeout => 220, die_on_timeout => 0);
	...
	script_run("date");
        ...
        script_run("date");
        script_run("sleep 79", timeout => 200, die_on_timeout => 0);
        script_run("date");
        assert_script_run("ssh-keyscan 127.0.0.1 | tee -a ~{,$user}/.ssh/known_hosts");
        script_run("date");
        script_run("sleep 83", timeout => 200, die_on_timeout => 0);
        script_run("date");
        assert_script_run("ssh-keyscan ::1 | tee -a ~{,$user}/.ssh/known_hosts");
        script_run("date");
        script_run("sleep 89", timeout => 200, die_on_timeout => 0);
        script_run("date");
    }
